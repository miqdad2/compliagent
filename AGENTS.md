...
Human reviewers focus on flagged findings only. Automatically verified findings are available for spot-check. Final approval remains with the responsible engineer or reviewer.

The primary output is a clause-by-clause compliance matrix and compliance report. The annotated PDF is an optional export enhancement — not the primary deliverable and not a required client-facing stage.
